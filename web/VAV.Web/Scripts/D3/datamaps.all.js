(function () {
    ...
    function handleGeographyConfig() {
        ...
        var mapId = $(this.options.element).attr('id'); 
        ...
        .on('mousedown', function (t, e) {
            ...
            if (options.onClick) {
                options.onClick(t.properties.name);
            }
            ...
            .attr('x', JSON.stringify(previousAttributes));
    ...
    ***************************************/
    ...
    Datamap.prototype.HighlightArea = function (id) {
        var self = this;
        var options = self.options.geographyConfig;
        var mapId = $(self.options.element).attr('id'); 
        svg = self.svg;
        var x = $('#' + mapId + ' [x]');
        if (x.length > 0) {
            var p = JSON.parse(x.attr('x'));
        ...
        svg.selectAll('.datamaps-subunit').each(
          function (c, index) {
              if (id.trim() == c.properties.name) {
                  var target = d3.select(this);
                  var previousAttributes = {
                      'fill': target.style.apply(target, ['fill']),
                      'stroke': target.style('stroke'),
                      'stroke-width': target.style('stroke-width'),
                      'fill-opacity': target.style('fill-opacity')
                  };
                  target.style('fill', options.highlightFillColor)
                    .style('stroke', options.highlightBorderColor)
                    .style('stroke-width', options.highlightBorderWidth)
                    .style('fill-opacity', options.highlightFillOpacity)
                    .attr('x', JSON.stringify(previousAttributes));
