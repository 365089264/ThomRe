...
namespace VAV.Web.Common
{
    public class MimeHelper
    {
        ...
        {
            //ILog _log = LogManager.GetLogger(typeof(MimeHelper));
            try
            {
                //_log.Error("First GetMimeFromBytes");
                uint mimeType;
                FindMimeFromData(0, null, data, (uint)MimeSampleSize, null, 0, out mimeType, 0);
                //_log.Error("Second GetMimeFromBytes(mimeType):" + mimeType);
                var mimePointer = new IntPtr(mimeType);
                //_log.Error("Third GetMimeFromBytes(mimePointer):" + mimePointer);
                var mime = Marshal.PtrToStringUni(mimePointer);
                //_log.Error("Fourth GetMimeFromBytes(mimePointer,mime):" + mimePointer + "," + mime);
                Marshal.FreeCoTaskMem(mimePointer);
                //_log.Error("Success GetMimeFromBytes");

                return mime ?? DefaultMimeType;
            }
            catch(Exception )
            {
                //_log.Error("Fail GetMimeFromBytes:" + ee.Message);
