...
{
    public class JsonTableGroupExcelResult: ActionResult
    {
        ...
        private List<string> _isSumRowColumnName;
        private string _sumGroupColumnName = string.Empty;
        ...
        public JsonTableGroupExcelResult(JsonExcelParameter parameter)
        {
            ...
            if (!string.IsNullOrEmpty(_sumGroupColumnName)) GroupJtable(_sumGroupColumnName, _isTotal, _isSumRowColumnName);
            BuildStyle();
        }
        ...
        private void GroupJtable(string categoryType, bool isTotal, List<string> totalColumns)
        {
            var groupList = new List<Dictionary<string, string>>();
            if (string.IsNullOrEmpty(categoryType) || _jTable.RowData.Count == 0)
            {
                return ;
            }
            var totalRow = new Dictionary<string, string>();
            foreach (var columnTemp in _jTable.ColumTemplate)
            {
                totalRow[columnTemp.ColumnName] = "";
            }
            var categoryList = new List<string>();

            foreach (var item in _jTable.RowData.Where(item => !categoryList.Contains(item[categoryType])))
            {
                categoryList.Add(item[categoryType]);
            }

            if (isTotal)
            {
                totalRow["isSumItem"] = "true";
                totalRow[categoryType] = Resources.Global.Total;
            ...
            {
                var sumRow = new Dictionary<string, string>();
                foreach (var columnTemp in _jTable.ColumTemplate)
                {
                    sumRow[columnTemp.ColumnName] = "";
                }
                ...
                sumRow[categoryType] = category;
                var temp =new List<Dictionary<string, string>>();
                var index = 0;
                for (; index < _jTable.RowData.Count; index++)
                {
                    var item = _jTable.RowData[index];
                    if (item == null || item[categoryType] != category) continue;
                    item["isSumItem"] = "";
                    temp.Add(item);
                    foreach (var column in totalColumns)
                    {
                        sumRow[column] =
                            (Convert.ToDecimal(sumRow[column]) + Convert.ToDecimal(item[column].Replace(",", "")))
                                .ToString();
                        totalRow[column] =
                            (Convert.ToDecimal(totalRow[column]) + Convert.ToDecimal(item[column].Replace(",", "")))
                                .ToString();
                    }
                }
                foreach (var column in totalColumns)
                {
                    sumRow[column] = string.Format("{0:N2}", sumRow[column]);
                }
                groupList.Add(sumRow);
                groupList.AddRange(temp);
            }
            if (isTotal)
            {
                foreach (var column in totalColumns)
                {
                    totalRow[column] = string.Format("{0:N2}", totalRow[column]);
                }
                groupList.Add(totalRow);
            }
            _jTable.RowData = groupList;
        }
        ...
        private void BuildTable()
        {
            ...
            foreach (var row in _jTable.RowData)
            {
                ...
                foreach (var column in _jTable.ColumTemplate)
                {
                    ...
                    if (!string.IsNullOrEmpty(_sumGroupColumnName) && row["isSumItem"] == "true" )
                    {
                        if (colPointer == 0)
                        {
                            WriteCell(rowPointer, colPointer++, 1, 1, row[_sumGroupColumnName], cStyle);
                            continue;
                        }
                        if (_sumGroupColumnName == column.ColumnName)
                        {
                            WriteCell(rowPointer, colPointer++, 1, 1, "", cStyle);
                            continue;
                        }
                    }
                    var valueText = row[column.ColumnName].Replace("&nbsp;", " ");
